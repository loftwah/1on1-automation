...
ECR_REPO_NAME="1on1-automation"
IMAGE_TAG="latest"
DOCKERFILE_DIR="../" # Directory where Dockerfile is located
CLUSTER_NAME="1on1-automation"
SERVICE_NAME="1on1-automation-service"
...
if [ "$REPO_EXISTS" != "$ECR_REPO_NAME" ]; then
    ...
    1on1 "ECR repository $ECR_REPO_NAME created."
else
    1on1 "ECR repository $ECR_REPO_NAME already exists."
fi
